"""
generate_qupath_ate.py — Extract the morphology composite TIFF from zarr for QuPath.
====================================================================================
Reads the morphology channels at a configurable pyramid level, composites into an RGB
TIFF, and saves a scale-factor JSON for mapping QuPath pixel coordinates to the micron
coordinate system of obsm['spatial'].

Compositing is delegated to xenium_image.py, whose channel-name matching already
covers Atera's four-stain stack (DAPI; ATP1A1/CD45/E-Cadherin; 18S; AlphaSMA/Vimentin)
— spatialdata-io recovers those names from the OME XML of the chNNNN_<stain>.ome.tif
files, so the composite is name-aware rather than positional.

This is the DEFAULT annotation image. When a registered H&E is configured,
generate_qupath_he_ate writes a second image alongside it and the user may annotate
either; prepare_input_ate prefers the H&E GeoJSON when present.
"""
...
log = logging.getLogger("generate_qupath_ate")
...
log.info("Exporting QuPath morphology image for %s at pyramid level %d",
         sample_id, qupath_level)
...
meta = {
    "sample": sample_id,
    # Which coordinate space a GeoJSON drawn on this image lives in. prepare_input_ate
    # branches on this, so the two images can never be confused for one another.
    "annotation_space": "morphology",
