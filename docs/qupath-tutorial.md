...
| `decoupled` (no Headblock) | There is no `qupath_images` target — the image is instead **embedded in the contract h5ad** you provided (`uns["spatial"]`). Export it for QuPath with the helper described in [Get started → decoupled mode](getting-started.md#21-region-annotation-in-decoupled-mode), or annotate regions when you build the contract externally. |
